...
function waboot_theme_styles() {
	$theme = wp_get_theme();
    $theme_name = apply_filters("waboot_stylesheet_name",$theme->stylesheet);

	/* Load theme styles */
    wp_enqueue_style( 'font-awesome', waboot_locate_template_uri( 'assets/css/font-awesome.min.css' ), $theme['Version'], 'all' );
    wp_enqueue_style( 'main-style', waboot_locate_template_uri( "assets/css/{$theme_name}.css" ), array( 'font-awesome' ), $theme['Version'], 'all' );
	wp_enqueue_style( 'core-style', get_stylesheet_uri(), array( 'font-awesome' ), $theme['Version'], 'all' );
}
...
function waboot_editor_styles() {
    $theme_name = apply_filters("waboot_stylesheet_name",wp_get_theme()->stylesheet);
