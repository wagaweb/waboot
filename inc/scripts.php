...
function waboot_js_loader() {
	...
	if(WABOOT_ENV == "dev"){
		...
		wp_enqueue_script( 'waboot-helper', wbf_locate_template_uri( 'sources/js/waboot-helper.js' ), array('jquery'),false, true);
	}else{
		wp_enqueue_script( 'waboot-plugins', wbf_locate_template_uri( 'assets/js/plugins.min.js' ), array('jquery'),false, true);
		wp_enqueue_script( 'waboot', wbf_locate_template_uri( 'assets/js/waboot.min.js' ), array('jquery','waboot-plugins'),false, true);
        if(is_child_theme()){
            $child_js = wbf_locate_template_uri( 'assets/js/waboot-child.min.js' );
            if($child_js != "")
...
function waboot_mobile_js_loader() {
    if(wb_is_mobile()){
        if(WABOOT_ENV == "dev"){
            wp_enqueue_script( 'fastclick', wbf_locate_template_uri( 'sources/js/vendor-mobile/fastclick.js' ), array('jquery'),false, true);
            wp_enqueue_script( 'touchSwipe', wbf_locate_template_uri( 'sources/js/vendor-mobile/jquery.touchSwipe.js' ), array('jquery'),false, true);
	        wp_enqueue_script( 'waboot-helper-mobile', wbf_locate_template_uri( 'sources/js/waboot-helper-mobile.js' ), array('jquery','waboot-helper','offcanvas','fastclick','touchSwipe'),false, true);
        }else{
	        wp_enqueue_script( 'waboot-mobile-plugins', wbf_locate_template_uri( 'assets/js/plugins-mobile.min.js' ), array('jquery'),false, true);
	        wp_enqueue_script( 'waboot-mobile', wbf_locate_template_uri( 'assets/js/waboot-mobile.min.js' ), array('jquery','waboot','waboot-plugins','waboot-mobile-plugins'),false, true);
        }
    }
}
