...
/**
 * Removes theme layouts support from specific post types
 *
 * @modifiedBy Riccardo D'Angelo
 * @since 0.4.0
 * @return void
 */
function theme_layouts_remove_post_type_support() {
    ...
    if ( post_type_exists( 'ml-slider' ) )
        remove_post_type_support( 'ml-slider', 'theme-layouts' );

    if ( post_type_exists( 'revision' ) )
        remove_post_type_support( 'revision', 'theme-layouts' );

    if ( post_type_exists( 'nav_menu_item' ) )
        remove_post_type_support( 'nav_menu_item', 'theme-layouts' );
...
function theme_layouts_save_post( $post_id, $post ) {
	...
	elseif ( current_user_can( 'edit_post_meta', $post_id, $meta_key ) && $meta_value !== $new_meta_value ){
        var_dump("salvp");
        update_post_meta($post_id, "Layout", $new_meta_value);
        update_post_meta($post_id, "layout", $new_meta_value);
		//set_post_layout( $post_id, $new_meta_value );
    }
}
