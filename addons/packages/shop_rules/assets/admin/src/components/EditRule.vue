<template>
    <div class="loading" v-if="loading">
        <h1>Edit Rule #{{ $store.state.editingShopRuleId }}</h1>
        <p>Loading...</p>
    </div>
    <div id="edit-rule" v-else>
      <div class="shop-rule__title">
        ...
        <button class="button button-danger" v-on:click="deleteRule" :disabled="savingRule">Delete</button>
        ...
        <div class="shop-rule__panel wp-filter">
            <div class="shop-rule__form">
                ...
                <label>
                    <span class="shop-rule__label">Enabled Rule</span>
                    <input type="checkbox" v-model="editingData.enabled" />
                </label>
                <label>
                    <span class="shop-rule__label">Choose start and end date </span>
                    <Datepicker class="shop-rule__calendar" v-model="dates" range></Datepicker>
                </label>
                <label v-show="false">
                    <span class="shop-rule__label">Choose dates timezone</span>
                    <select v-model="editingData.timezone">
                        <option value="Europe/Rome" :selected="editingData.timezone === 'Europe/Rome'">Europe/Rome</option>
                    </select>
                </label>
                <label>
                    ...
                    <select v-model="editingData.type">
                        <option value=""></option>
                        <option v-for="t in pageData.shop_rule_types" :selected="editingData.type === t.slug" :value="t.slug" >{{ t.label }}</option>
                    </select>
                </label>
            </div>
        </div>

        <template v-if="currentRule !== null">
                ...
                <div class="shop-rule__form shop-rule__form--stacked" :key="index" v-for="(taxFilter, index) in editingData.taxFilters">
                    <label>
                        ...
                        <select v-model="editingData.taxFilters[index].taxonomy" @change="onFilterTaxonomyChange(index)">
                            <option :value="taxonomy.name" :selected="taxFilter.taxonomy === taxonomy.name" v-for="taxonomy in pageData.taxonomies">{{ taxonomy.label }}</option>
                        </select>
                    </label>
                    ...
                    <label>
                        ...
                        <select v-model="editingData.taxFilters[index].criteria">
                            <option :selected="taxFilter.criteria === 'in'" value="in">In</option>
                            <option :selected="taxFilter.criteria === 'not-in'" value="not-in">Not in</option>
                        </select>
                    </label>
                    ...
                    <div class="shop-rule__form shop-rule__form--stacked">
                        <label>
                            ...
                            <select v-model="editingData.joinTaxonomy.taxonomy" @change="onJoinTaxonomyChange()">
                                ...
                                <option :value="taxonomy.name" :selected="editingData.joinTaxonomy.taxonomy === taxonomy.name" v-for="taxonomy in pageData.taxonomies">{{ taxonomy.label }}</option>
                            </select>
                        </label>
                ...
                <div class="shop-rule__form">
                    <template v-if="editingData.type === 'buy-x-get-y'">
                        ...
                        <label>
                            ...
                            <select v-model="editingData.addToCartCriteria">
                                <option :selected="editingData.addToCartCriteria === criteria.slug" :value="criteria.slug" v-for="criteria in pageData.add_to_cart_criteria">{{ criteria.label }}</option>
                            </select>
                        </label>
                    ...
                    <template v-if="editingData.type === 'cart-adjustment'">
                        ...
                        <label>
                            ...
                            <small>The discount label must be unique across all rules</small>
                        </label>
                    </template>
                </div>
            </div>
...
export default defineComponent({
    ...
    methods: {
        async fetchRule(){
            let ruleData = await fetchRuleById(this.$store.state.editingShopRuleId);
            if(ruleData !== null){
                this.editingData = this.getEditingDataByRuleData(ruleData);
                this.dates = [new Date(this.editingData.from ?? ''), new Date(this.editingData.to ?? '')];
                if(ruleData.taxFilters.length > 0){
                    for(const taxFilter of ruleData.taxFilters){
                        this.editingData.taxFilters.push({
                            taxonomy: taxFilter.taxonomy,
                            criteria: taxFilter.criteria,
                            atLeastOne: taxFilter.atLeastOne,
                            terms: taxFilter.terms.map( (id: string): number => { return parseInt(id); })
                        });
                    }
                }
                if(typeof ruleData.joinTaxonomy !== 'undefined'){
                    this.editingData.joinTaxonomy = {
                        taxonomy: ruleData.joinTaxonomy.taxonomy,
                        term: ruleData.joinTaxonomy.term
                    }
                }
                if(typeof ruleData.products !== 'undefined' && ruleData.products.length > 0){
                ...
                if (typeof ruleData.discount !== 'undefined') {
                    Object.assign(this.editingData.discount, ruleData.discount);
                }
                this.currentRule = ruleData;
            }
        },
        addTaxonomyFilter(){
            let newFilter = {
        ...
        async saveRule(){
            ...
            let saved = await saveRule(this.editingData,this.$store.state.editingShopRuleId);
            if(saved){
                this.shopRuleSavedSuccessfully = true;
                setTimeout(() => { this.shopRuleSavedSuccessfully = false }, 2000);
            }else{
                this.failNotices.push('Error occurred during saving');
        ...
        async deleteRule(){
            this.savingRule = true;
            try{
                await deleteRule(this.$store.state.editingShopRuleId);
                this.shopRuleSavedSuccessfully = true;
                setTimeout(() => { this.shopRuleSavedSuccessfully = false; this.$store.commit('goToListView'); }, 2000);
            }catch (e){
                this.failNotices.push('Error occurred during deletion');
                setTimeout(() => { this.failNotices = [] }, 2000);
            }
            this.savingRule = false;
        },
